...
| 持ち物（インベントリ）＋装備ドロップ3択＋付け外しUI | 🟢 `InventoryService`/`BrigadeInventory`（永続化 v5）・`EquipmentDropService`→`EquipmentDropOverlay`・拠点の「🎒 持ち物」 |
...
| 残課題 | ▶ `ProphecyMaster` 本実装 / Affix拡張（経済軸・パッシブ付与） / 背景・敵アセット / 年送り反映後のバランス再検証 |
