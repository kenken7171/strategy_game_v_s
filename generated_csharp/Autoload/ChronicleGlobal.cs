...
public partial class ChronicleGlobal : Godot.Node
{
    ...
    // ────────────────────────────────────────────────────────────────────────
    //  編成/拠点: 装備の直接脱着（無償・ポイント経済を消費しない）
    // ────────────────────────────────────────────────────────────────────────
    //  設計憲法③「単一 SoT・単方向データフロー」。兵器廠（ExecuteBuyEquipment /
    //  ExecuteUpgradeEquipment）が「共通サイフを消費して新品を購入・強化する」のに対し、
    //  以下 2 API は「旅団が所有する装備個体を、持ち物（BrigadeInventory）とユニットの間で
    //  非破壊に往復させる」無償の付け外しを担う。FormationUI の装備ドック／拠点 MarriageUI の
    //  「🎒 持ち物」が直接叩く窓口。純粋ロジックは InventoryService (Core/Units) に分離済み。
    //  （旧・無からの conjure／外すと discard だった EquipItem/UnequipItem は持ち物導入で廃止。）
